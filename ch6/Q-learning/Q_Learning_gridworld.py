...
class Agent:
    ...
    def update_Q_table(self, state, action, reward, next_state):
        # Q-learning 업데이트
        optimal_action = self.OptimalAction(next_state)
        self.Q_table[state][action] += (
                self.alpha * (
                    reward + self.gamma * self.Q_table[next_state][optimal_action] - self.Q_table[state][action]))
...
def QLEARNING_Train(env, agent, epsilon, num_episodes):
...
if __name__ == '__main__':
    ...
    QLEARNING_Train(env, agent, 1.0, 1000)
